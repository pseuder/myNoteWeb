 ...
 */
export const fetchSecrets = async (): Promise<Secret | null> => {
  try {
 ...
 */
export const saveSecrets = async (content: string): Promise<number> => {
  try {
    const response = await axios.post(`${API_BASE_URL}/secrets`, { content });
